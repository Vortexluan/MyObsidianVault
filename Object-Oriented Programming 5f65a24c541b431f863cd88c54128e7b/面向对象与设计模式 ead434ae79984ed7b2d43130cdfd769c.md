...
![Untitled|697](%E9%9D%A2%E5%90%91%E5%AF%B9%E8%B1%A1%E4%B8%8E%E8%AE%BE%E8%AE%A1%E6%A8%A1%E5%BC%8F%20ead434ae79984ed7b2d43130cdfd769c/Untitled%208.png)
需要注意的是这里的数值对象不是数据库也不是什么用来充当数据库的.txt文件，它就是一个用来搬运数据的类，运行在内存里的用来搬运打包数据的纸箱子。

事实上，这里要调用DAOimpl的上下文和实际操作的数据库都没有表示出来。
至于为什么DaoImpl到ValueObject用的是实线箭头，注意DaoImpl的那个List成员变量，这个一般都有。
### 观察者模式
...
![[Pasted image 20260607151557.png|693]]
这里的update()是subject根据observer的list找到observer，然后利用update进行更新。
### 模板方法模式
