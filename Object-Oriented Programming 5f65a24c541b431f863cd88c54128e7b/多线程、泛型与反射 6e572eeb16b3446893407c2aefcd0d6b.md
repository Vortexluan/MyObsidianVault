...
不能直接调用run()的原因是run()不会另开线程，只会在当前线程去另外执行任务，然后再回来做应该做的事情。
...
- 等待状态 `WAITING`，在当前线程调用 `Object.wait()` 或者其他线程调用 `Thread.join()` 且没有设置时间时，进入等待状态。
    ...
    当调用了 `Thread.join()` 的线程结束，或调用 `Object.notify()` 或 `Object.notifyAll()` 来唤醒等待中的线程时，线程返回可运行状态。注意t1.join()是让一个程序去等待t1执行完毕而不是代替t1执行。
...
### 线程的计时
注意，这里的Object都表示对应的锁，我们也可以理解为什么说wait()要写在同步块里面，因为wait要拿到对应的锁

运行->等待
1.当前线程调用 Object.wait()
2.其它线程调用 Thread.join()

等待->运行
1.正在等待的线程被其它方法唤醒 Object.notify()或者Object.notifyall()
2.调用的Thread.join()结束

### 线程的计时/超时等待
1.当前线程对象调用 Object.wait(time)
2.其它线程调用 Thread.join(time)
3.当前线程调用 Thread.sleep(time)

所以我们也可以理解wait()和sleep()之间的异同点，前者是Object的方法，而Object又是一个锁
sleep不释放锁，是一个Thread方法，wait释放锁，是一个Object方法
...
对一个线程调用 `interrupt()` 方法可以将线程的中断标记设置为 `true`，这个标志是每一个线程都应该时不时查看的。线程可以自己决定是否处理这个中断，并不是强制的，只是引起线程的注意。
值得注意的是，如果在一个已经阻塞的线程上面调用interrupt方法，对应的阻塞调用将会被InterruptedException中断

### 线程优先级
值得一提的是，每个线程都有一个继承过来的优先级，但这个优先级只是意味着被执行的概率的大小，不意味着具体的确定的顺序。
...
    synchronized (Counter.lock) { // 获得锁对象

        // 此时其他线程无法进入这个同步块

        Counter.count++; // 递增正确的计数器变量
...
我怎么感觉这里没有必要弄一个图，用mermaid就能够直接做到
...
原因是java会自动帮你把事情干了，当然代码逻辑是必须要能够运行的。
...
因为泛型类的泛型是要在实例创建的时候才能够确定的，静态的泛型方法是在类就有的，显然不能够使用一个完全没有确定的泛型。
...
形参指的是函数定义的时候的参数。实际对实例传入的参数那个叫做实参
